	...
	new Vue({
		...
		methods:{
			search(){
				...
				var labels = [];
				var start = this.getWeekOfYear(new Date(startDate));
				var end = this.getWeekOfYear(new Date(endDate));
				
				var url = '';
				if(this.$data.btn == '事故类型'){
					url = '/safety/Statistics/weekOffgradeTroubleCount';
				}else if(this.$data.btn == '风险级别'){
					url = '/safety/Statistics/weekOffgradeLevelCount';
				}else{
					url = '/safety/Statistics/weekOffgradeHarmfulCount';
				}

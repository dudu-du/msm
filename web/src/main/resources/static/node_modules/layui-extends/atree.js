...
layui.define('jquery', function(exports) {
  ...
  var enterSkin = 'layui-atree-enter',
      Atree = function(options) {
        ...
        this.editBtnLabelKey = this.props.editBtnLabel || props.editBtnLabel;
  ...
  Atree.prototype.getDom = function(item) {
    var that = this.$vm,
        ...
        showTag = hasChild ? 'layui-show' : 'layui-hide';
    // console.log(item);
    return {
      spread: function() {
        return hasChild ? '<i class="layui-icon layui-atree-spread">' + (
            item[that.spreadKey] || options.spreadAll ? icon.arrow[1] : icon.arrow[0]
        ) + '</i>' : '<i class="layui-icon layui-atree-spread layui-atree-null"></i>';
      },
      checkbox: function() {
        if(item.orgType == 'OT_BUREAU'){
          return options.check ? (
              '<i class="iconfont icon-zhengfubaozhang icon-tree-welsee icon-tree-welsee-orange"></i>'
          ) : '';
        }else{
          return options.check ? (
              '<i class="iconfont icon-dalou3 icon-tree-welsee"></i>'
          ) : '';
        }
      },
      /*checkbox: function() {
      ...
      node: function() {
        return '<a title="'+item.code+'" href="' + (item.href || 'javascript:;') + '" ' + (
      ...
      menu: function() {
        return '<div class="layui-atree-menu">' +
            ...
            '<span class="layui-atree-edit">' + that.editBtnLabelKey + '</span>' +
  ...
  Atree.prototype.getNode = function(item) {
    ...
    '><div class="layui-atree-node" data-id="'+item.id+'" data-code="'+item.code+'" data="'+domTag+'">'
      //展开箭头
      ,dom.spread()
      //复选框
      ,dom.checkbox()
  ...
  Atree.prototype.bindUlEvent = function(li, item) {
    ...
    //编辑方法
    typeof options.editClick === 'function' && that.edit(li, item);
    //删除方法
  ...
  //编辑方法回调
  Atree.prototype.edit = function(elem, item) {
    var that = this.$vm,
        options = this.$options;
    var node = elem.children('.layui-atree-node');
    var editBtn = node.children('.layui-atree-menu').children('.layui-atree-edit')
    var arrow = node.children('.layui-atree-spread');
    var ul = elem.children('ul'),
        a = node.children('a');
    var editEvent = function(e) {
      layui.stope(e);
      var _editEvent = {
        edit: function(itemAddObj) {
          $(elem).children('.layui-atree-node').find('cite').text(itemAddObj);
          $(elem).children('.layui-atree-node').find('a').attr('title',item.code);
        }
      }
      options.editClick(item, elem, _editEvent.edit)
    }
  ...
  Atree.prototype.delete = function(elem, item) {
    ...
    var deleteEvent = function(e) {
      ...
      var _deleteEvent = {
          ...
          var arrow = parent.parent().children('.layui-atree-node').find('.layui-atree-spread');

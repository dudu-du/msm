...
layui.define(['jquery', 'table'], function(exports) {
	...
	yutons_sug.prototype.render = function(opt) {
		...
		//初始化输入框提示容器
		$("#" + opt.id).after("<div id='sugItem' style='background-color: #fff;display: none;z-index:1;position: absolute;width:100%;max-height: "+opt.height+"px; overflow: auto;'></div>");
		...
		var thisDome = $("#" + opt.id),
			thisDomeParent = thisDome.parent().parent();
		$(document).bind('touched mousedown',function(e){
			if(!thisDomeParent.is(e.target)&&thisDomeParent.has(e.target).length===0){
				thisDome.next().hide().html("");
			};
		});
		if (opt.type == "sugTable") {
			...
			//如果type为sug，则初始化下拉框
			$("#" + opt.id).next().css("border", "solid #e6e6e6 1px");
			/* 输入框鼠标松开事件 */
			$("#" + opt.id).mouseup(function(e) {
				opt.obj = this;
				getSug(opt);
	...
	function getSug(opt) {
		...
		var kw = $.trim($(opt.obj).val()),nm=opt.datajson.name,vl=opt.datajson.value;
		if (kw == "") {
			$("#" + opt.id).next().hide().html("");
			return false;
		}
		...
		$.ajax({
			...
			success: function(data){
				...
				layui.each(data.data, function(index, item) {
						html +="<div class='item' style='padding: 3px 10px;cursor: pointer;' data='"+item[vl]+"' onmouseenter='getFocus(this)' onClick='getCon(this,"+opt.datajson.name+","+opt.datajson.value+");'>" +
							item[nm] + "</div>";
				});
				if (html != "") {
	...
	window.getCon = function(obj,nm,val){
		console.log(obj,nm,val);
		var name = $(obj).text(),
			value = $(obj).attr('data');
		$(nm).val(name);
		$(val).val(value);

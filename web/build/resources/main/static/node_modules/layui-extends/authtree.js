...
layui.define(['jquery', 'form'], function(exports){
    ...
    obj = {
        ...
        iconToggle: function(dst, iconobj){
            ...
            if(origin.is('.active')){
                /*收起*/
                console.log(origin.parent('.auth-status').attr('data-i'))
        ...
        renderAuth: function(tree, dept, opt){
            ...
            layui.each(tree, function(index, item){
                //console.log('当前数组index:  '+dept+'-'+index,'  层级index:  '+dept,'  当前数据:  '+item.name);
                // console.log(item);
                var hideC = dept == 2?'layui-hide':'';
                var itemJosn = JSON.stringify(item);
                opt.id = item.id;
                var hasChild = item.childOrg.length>0 ? 1 : 0;
                // 注意：递归调用时，this的环境会改变！
                var append = hasChild ? obj.renderAuth(item.childOrg, dept+1, opt) : '';
                var openstatus = openall || (opt.openchecked && item.checked);

                // '+new Array(dept * 4).join('&nbsp;')+'
                str += '<div><div class="auth-status" data-i="'+index+'" data-dept="'+dept+'" style="display: flex;flex-direction: row;align-items: flex-end;"> '+
                    "<div class='tree-hide-btn "+hideC+"' data-dept='"+dept+"'>"+
                    '<a href="javascript:void(0);" class="tree-icon tree-add" title="添加" name="add" lay-data="'+item.value+'"><i class="iconfont icon-zengjia1"></i></a>' +
                    '<a href="javascript:void(0);" class="tree-icon tree-edit" title="编辑" name="edit" lay-data="'+item.value+'"><i class="iconfont icon-bianji"></i></a>' +
                    '<a href="javascript:void(0);" class="tree-icon tree-del" title="删除" name="del" lay-data="'+item.value+'"><i class="iconfont icon-shanchu1"></i></a>' +
                    '</div>'+
                    (hasChild?'<i class="layui-icon tree-lay-icon auth-icon '+(openstatus?'active':'')+'" style="cursor:pointer;">'+(openstatus?obj.openIconContent:obj.closeIconContent)+'</i>':'<i class="layui-icon auth-leaf" style="opacity:0;">&#xe626;</i>')+
                    // (dept > 0 ? '<span>├─ </span>':'')+
                    (dept > 0 ? ' ':'')+
                    '<input type="checkbox" name="'+inputname+'" title="'+item.name+'" value="'+item.id+'" lay-skin="primary" lay-filter="'+layfilter+'" '+
                    (item.checked?'checked="checked"':'')+'>' +
                    ' </div>'+
                    ' <div class="auth-child" style="'+(openstatus ?'':'display:none;')+'padding-left:26px;"> '+append+'</div></div>'
            });
            str += '</div>';
            return str;
        },

...
layui.use(['element','table','layer','form', 'upload'], function(){
    ...
    var url = origin + '/View/listSchoolAdminInfo';
    var getData = {};
        ...
        switch(obj.event){
            ...
            case 'del':
                ...
                layer.confirm('确认要删除该数据吗？', function(index){
                    $.ajax({
                        url: origin + "/View/schoolAdmin?id="+checkedData,
                        type: "DELETE",
                        success: function (data) {
                            if(data.success){
                                layer.close(index);
    ...
    table.on('tool(test)', function(obj){
        ...
        if(obj.event === 'del'){
            layer.confirm('确认要删除该数据吗？', function(index){
                $.ajax({
                    url: origin + "/View/schoolAdmin?id="+data.id,
                    type: "DELETE",
                    success: function (data) {
                        if(data.success){
                            console.log(data);
	...
	var addSchoolHtml = '<div class="lay-mask-box">' +
			...
			'</div>'+*/
			'</div>';
		function addMaskSchool(tag,data){
			var title;
			if(tag == 'add'){
				title = '添加校管理员'
			}else{
